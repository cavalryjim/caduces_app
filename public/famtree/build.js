...
function BCD(d,ds,aw,cx,cy){
TAL(d,cx,cy-1,cx,cy-0.5,false);
TAL(d,cx-aw/2,cy-0.5,cx+aw/2,cy-0.5,false);
...
TAL(d,fx,cy,(pi&&f[pi])?px:cx,cy,FMS(f,i,pi));
}
...
var ds=BCG(f,tc,h-1);
if(ac.length){
sx=sr?(d.r+ds.tw/2):(d.l-ds.tw/2-1);
}
BCD(d,ds.ds,ds.aw,sr?(sx-0.5):(sx+0.5),1);
}
TAL(d,0,0,sx,0,FMS(f,i,p.es));
BAE(d,f,p.es,i,sx,0,true,sr,true);
}
for(var pi in p.pc){
if(pi!=p.es){
BDH(d,f,i,pi,FLP(f,i,pi),h-1,!sr,0,0);
}
}
}else{
...
var ac=FLA(f,p.m);
if(ac.length&&p.f){
BDH(d,f,p.m,null,ac,co,false,mx,-1);
}
for(var pi in pm.pc){
if(pi!=p.f){
BDH(d,f,p.m,pi,FLP(f,p.m,pi),co,false,mx,-1);
}
}
}
if(p.f){
var ac=FLA(f,p.f);
if(ac.length&&p.m){
BDH(d,f,p.f,null,ac,co,true,fx,-1);
}
for(var pi in pf.pc){
if(pi!=p.m){
BDH(d,f,p.f,pi,FLP(f,p.f,pi),co,true,fx,-1);
}
}
}
if(p.m){
